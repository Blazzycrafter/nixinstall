...
# 2. Generiere Hardware-Konfiguration für das Zielsystem, falls im Repo keine liegt
if [[ ! -f "$REPO_DIR/hardware-configuration.nix" ]]; then
  echo "[STEP] Keine hardware-configuration.nix im Repo. Generiere eine spezifische für dieses System..."
  TMP_DIR=$(mktemp -d)
  sudo nixos-generate-config --dir "$TMP_DIR" --root /mnt
  sudo cp "$TMP_DIR/hardware-configuration.nix" "$REPO_DIR/hardware-configuration.nix"
  sudo chown "$USER:$USER" "$REPO_DIR/hardware-configuration.nix"
  sudo rm -rf "$TMP_DIR"
  echo "[OK] hardware-configuration.nix wurde generiert."
fi
...
# hardware-configuration.nix kopieren
if [[ -f "$REPO_DIR/hardware-configuration.nix" ]]; then
